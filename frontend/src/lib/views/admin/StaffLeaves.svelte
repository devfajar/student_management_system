<script>
  ...
  async function loadLeaves() {
    loading = true;
    try {
      leaves = await api.getStaffLeaves();
    } catch (err) {
      error = err.message || 'Failed to load staff leaves';
    } finally {
      loading = false;
    }
  }
  ...
  async function handleApprove(id) {
    try {
      await api.approveStaffLeave(id);
      success = 'Staff leave approved!';
      await loadLeaves();
    } catch (err) {
  ...
  async function handleDisapprove(id) {
    try {
      await api.disapproveStaffLeave(id);
      success = 'Staff leave disapproved!';
      await loadLeaves();
    } catch (err) {
