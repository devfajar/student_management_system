<script>
  ...
  async function loadFeedback() {
    loading = true;
    try {
      feedbacks = await api.getStudentFeedback();
    } catch (err) {
      error = err.message || 'Failed to load feedbacks';
    } finally {
      loading = false;
    }
  }
  ...
  async function handleSendReply(e) {
    ...
    error = '';
    try {
      await api.replyStudentFeedback(activeFeedback.id, reply_message);
...
<Modal show={showModal} title="Reply to Student Feedback" onclose={() => showModal = false}>
  <form onsubmit={handleSendReply}>
    <div class="feedback-context">
      <p><strong>Student:</strong> {activeFeedback?.student_name}</p>
      <p><strong>Message:</strong> {activeFeedback?.feedback}</p>
    </div>

    <div class="form-group">
      <label class="form-label" for="fb-reply">Your Reply</label>
      <textarea id="fb-reply" class="form-control" bind:value={reply_message} placeholder="Type your reply here..." rows="4" required></textarea>
    </div>
