...
<div class="space-y-6 animate-in fade-in duration-200">
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-2xl font-bold text-slate-800 tracking-tight flex items-center gap-2.5">
        ...
        Staff Announcements & Circulars
      </h1>
      <p class="text-sm text-slate-500 mt-1">Official institutional directives, circulars, and notifications for faculty</p>
    </div>
  ...
  <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
    {#if loading}
      <div class="p-12 text-center text-slate-400">Loading faculty notifications...</div>
    {:else if notifications.length === 0}
      <div class="py-16 text-center text-slate-400 space-y-2">
        ...
        <p class="text-xs text-slate-400">You have no pending announcements or faculty notices.</p>
      </div>
    {:else}
