<script>
  ...
  async function loadFeedback() {
    loading = true;
    try {
      feedbackHistory = await api.getStaffFeedback();
    } catch (err) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      await api.sendStaffFeedback(feedbackText);
  ...
  <div class="card">
    ...
    <form onsubmit={handleSubmit}>
      <div class="form-group">
        <label class="form-label" for="stf-fb-text">Feedback Message</label>
        <textarea
          id="stf-fb-text"
          class="form-control"
          bind:value={feedbackText}
          placeholder="Share thoughts, suggestions, or concerns with the administration..."
          rows="4"
          required
        ></textarea>
      </div>
